...
local DiscordLink = Inicio:AddDiscordInvite({
	Name = "Discord Bo May Ne Cu",
	Logo = "https://cdn.discordapp.com/avatars/1247437589670264883/a_633edda3778de5667894af952c174f3f.png",
	Invite = "https://discord.gg/uXvAdBTx"
})
...
if Sea3 then
  ...
  local TimeKitsune = AutoSea:AddParagraph({"🧭 Chờ Nó Tí Kitsune Island"})
spawn(function()
        pcall(function()
            while wait() do
                if game:GetService("Players").LocalPlayer.PlayerGui.Main.Timer.Visible == true then
                    ...
                    TimeKitsune:SetTitle("🧭 Chờ Nó Tí Kitsune Island")
                end
            end
        end)
    end)
  AutoSea:AddToggle({Name = "Tự Động Dí Kitsune Island",Callback = function(Value)
...
function UpdateIslandKisuneESP() 
  ...
  task.spawn(function()
    ...
    while task.wait() do
      if Map:FindFirstChild("KitsuneIsland") then
        KILabel:SetTitle("🎇 Check Kitsune Island : Có Spawned✅ | Distance : " .. Distance)
      else
        KILabel:SetTitle("🎇 Check Kitsune Island : Dell Spawned❌")
      end
    end
  end)
    ...
    pcall(function()
        while wait() do
            if game.Workspace._WorldOrigin.Locations:FindFirstChild('Frozen Dimension') then
                FrozenIsland:SetTitle("♾️ Check Frozen Dimension : Có Rồi Cu Spawned✅")
      else
        FrozenIsland:SetTitle("♾️ Check Frozen Dimensiin : Chưa Có Spawned❌")
            end
        end
    end)
end)
...
AutoSea:AddToggle({
	Name = "Auto Kill Leviathan (Dell Biết Dùng Đc K :)",
	Default = false,
	Callback = function(Value)
...
AutoSea:AddToggle({
	Name = "Teleport to Frozen Dimension (Mong Mày Đến Muộn Để Dell Bú Đc)",
	Default = false,
	Callback = function(Value)
...
MirageTab:AddToggle({
	Name = "Auto Teleport To Advanced Fruit Dealer(Chắc K Có Trái J Đâu)",
	Default = false,
	Callback = function(Value)
...
MirageTab:AddToggle({
	Name = "Auto Collect Blue Gear (K bt nhặt đc không)",
	Default = false,
	Callback = function(Value)
...
MirageTab:AddToggle({
	Name = "Auto Lock Cam to Moon+Enable Race Ability(Bật Nhiều Quá Lag Mắt)",
	Default = false,
	Callback = function(Value)
...
MirageTab:AddToggle({
	Name = "Auto Collect Chest Mirage(Banned)",
	Default = false,
	Callback = function(Value)
...
local ScreenGui = Instance.new("ScreenGui")
    ...
    MoonImg.Image = ""rbxassetid://1234567890""
...
MirageTab:AddDropdown({
	Name = "Select Gear",
	Options = {"Gear1", "Gear2" , "Gear3","Gear4","Gear5" "Gear6 (Ai Ấn Vô Là Thk Nguu)"},
	Default = "",
	Flag = "NahGearj",
	Callback = function(Value)
    ...
    task.spawn(function()
      ...
      while getgenv().AutoFinishTrial do task.wait()
        if PlayerRace and typeof(PlayerRace) == "string" then
          if PlayerRace == "Cyborg(Ng Máy)" then
            PlayerTP(CFrame.new(28654, 14898, -30))
          elseif PlayerRace == "Ghoul(Quỷ)" then
            KillAura()
          elseif PlayerRace == "Human(Người)" then
            KillAura()
          elseif PlayerRace == "Mink(Thỏ)" then
          elseif PlayerRace == "fish(Cá)" then  
           ...
           for i,v in pairs(game:GetService("Workspace").SeaBeasts:GetChildren()) do 
						if v.Name ==  "SeaBeast" and v:FindFirstChild("HumanoidRootPart") then
							...
							for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
								if v:IsA("Tool") then
									if v.ToolTip == "Wear" then -- "Blox Fruit" , "Sword" , "Wear" , "Agility"
										game.Players.LocalPlayer.Character.Humanoid:EquipTool(v)
									end
								end
							end
...
MainFarm:AddToggle({
  Name = "Auto Chest Bypass (Dễ Reset Acc) ",
  Callback = function(Value)
        ...
        while getgenv().AutoFarmRaid do task.wait()
          if Configure("Raid") then
            ...
            if not Player.PlayerGui.Main.Timer.Visible and VerifyTool("Special Microchip") then
              ...
              and not GetIsland("Island 5 is Winer") then
                pcall(function()
                  if Sea2 then
...
elseif Sea2 then
  ...
  QuestsTabs:AddToggle({
    Name = "Auto Shisui",
    Callback = function(Value)
      getgenv().AutoShisui = Value
      AutoShisui()
    end
  })

  QuestsTabs:AddToggle({
    Name = "Auto Shisui Hop",
    Callback = function(Value)
      getgenv().AutoShisuiHop = Value
    end
  })
...
table.insert(AFKOptions, Misc:AddToggle({"Remove Notifications", true, function(Value)
...
Shop:AddButton({"Shisui", function()FireRemote("CyborgTrainer", "Buy")end})
Shop:AddButton({"Wando", function()FireRemote("CyborgTrainer", "Buy")end})
Shop:AddButton({"Saddi", function()FireRemote("CyborgTrainer", "Buy")end})
...
Visual:AddToggle({Name = "ESP Islands",Callback = function(Value)
  ...
  Visual:AddSection({"Troll Viey Nammm "})
...
Visual:AddSection({"Divine Art Your Troll"})
Visual:AddDropdown({
	Name = "Select Quest",
  ...
  Visual:AddButton({
    ...
    Callback = function()
      ...
      local ArgsTransform = {
        Character = game.Players.LocalPlayer.Character,
        CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame,
        Color1 = Color3.fromRGB(0, 100, 255), -- Màu chủ đạo xanh nước biển
        Color2 = Color3.fromRGB(0, 150, 255), -- Màu thứ hai
        Color3 = Color3.fromRGB(0, 200, 255), -- Màu thứ ba
      }
      Player.Character.Humanoid:LoadAnimation(ReplicatedStorage.Util.Anims.Storage["2"].RaceTransform):Play()
...
task.spawn(function()
	Fluent:Notify({
		Title = "Light Hub",
		Content = "Light Hub Loaded!",
		SubContent = "https://discord.gg/uXvAdBTx",
		Duration = 3
	})
end)
